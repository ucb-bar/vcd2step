...
int main(int argc, const char **argv)
{
    ...
    fprintf(v, "  reg clk = 0;\n");
    fprintf(v, "  reg reset = 0;\n");

    for (const auto& signal: signals) {
        auto inp = should_poke.find(signal.name) == should_poke.end();
        if (signal.width == 1) {
            fprintf(v, "  %s %s;\n",
                    inp ? "reg" : "wire",
                    signal.name.c_str()
    ...
    fprintf(v, "  always #`CLOCK_PERIOD clk = ~clk;\n");
    ...
    for (const auto& signal: signals) {
        fprintf(v, "    $readmemh(\"@@TEST_BASE_DIR@@/%s.dat\", __list__%s);\n",
                signal.name.c_str(),
                signal.name.c_str()
            );
    }
    ...
    for (const auto& signal: signals) {
        ...
        fprintf(v, "      if (%s != __list__%s[cycle-1])\n",
                signal.name.c_str(),
                signal.name.c_str()
            );
